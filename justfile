# frontmatter-mcp — baseline + frontmatter-mcp symlink
...
install *flags: compress
    ...
    link="$dir/frontmatter-mcp"
    if [ ! -e "$link" ] && [ ! -L "$link" ]; then
        $sudo ln -s "frontmatter" "$link"
        echo "linked $link -> frontmatter"
    fi
...
uninstall *flags:
    ...
    $sudo rm -f "$dir/frontmatter-mcp"
    echo "removed $dir/frontmatter-mcp"
